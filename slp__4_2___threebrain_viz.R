...
#### Configuration ----
setwd('Z:/George/Projects/Claustrum')
...
input <- list(
  list(number = 1, 
       ...
       selected_path = 'Cache/Subject01/S01_electrodes.csv'), 
#  list(number = 2, 
#       elec_path = 'Cache/Subject02/S02_electrodes_tkrRAS.csv', 
#       selected_path = 'Cache/Subject02/S02_electrodes.csv'),
  list(number = 5, 
       elec_path = 'Cache/Subject05/S05_electrodes_tkrRAS.csv', 
       selected_path = 'Cache/Subject05/S05_electrodes.csv')
)
...
for (subject in input) {
  ...
  elec_values_temp <-
    ...
    filter(status == 'accept' & type == 'macro') %>%
  ...
  elec_values_temp <- elec_values_temp[, c('Electrode', 'roi_1')]
